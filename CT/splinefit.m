...
%
%   OPTIONAL INPUT
%   Argument places 4 to 7 are reserved for optional input.
...
% Check arguments
[x,y,dim,breaks,n,periodic,robust,constr] = arguments(varargin{:});
...
% Bin data
[junk,ibin] = histc(x,[-inf,breaks(2:end-1),inf]); % ibin:label piece of x
                                               % junk: number x in n piecees

% Sparse system matrix
mx = numel(x); % tot num of x
ii = [ibin; ones(n-1,mx)];
ii = cumsum(ii,1);  %?? index of sparse matrix
jj = repmat(1:mx,n,1); %??
if periodic
...
%--------------------------------------------------------------------------
function [x,y,dim,breaks,n,periodic,robust,constr] = arguments(varargin)
...
%   robust      True if robust fitting scheme
