...
function [A,normA] = defineParameterMax_diagonal2(image)
...
%Imax=zeros(Nx,Ny);
%Imin=zeros(Nx,Ny);
imageflip=flipdim(image,2);

for k=-(Nx-1)+3:(Ny-1)-3
    diagonal = transpose(diag(imageflip,k));
...
%             end
           if (k < 0)
                A(Nx*(Ny-(i-1)-1)-k+i,Nx*(Ny-(ind(j)-1)-1)-k+ind(j)) = 1;
                A(Nx*(Ny-(i-1)-1)-k+i,Nx*(Ny-(ind(j-1)-1)-1)-k+ind(j-1)) = d1(j)/d(j);
                A(Nx*(Ny-(i-1)-1)-k+i,Nx*(Ny-(ind(j+1)-1)-1)-k+ind(j+1)) = d2(j)/d(j);
                %A(-k+i,ind(j)) = 1;
                %A(-k+i,ind(j-1)) = d1(j)/d(j);
                %A(-k+i,ind(j+1)) = d2(j)/d(j);
            else A(Nx*(Ny-k-i)+i,Nx*(Ny-k-ind(j))+ind(j)) = 1;
                A(Nx*(Ny-k-i)+i,Nx*(Ny-k-ind(j-1))+ind(j-1)) = d1(j)/d(j);
                A(Nx*(Ny-k-i)+i,Nx*(Ny-k-ind(j+1))+ind(j+1)) = d2(j)/d(j);
                %A(i,k+ind(j)) = 1;
...
save('matriceDdiagonal2.mat','A','normA');