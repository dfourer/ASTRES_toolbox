function  [A,normA, alpha,beta, ind] = defineParameterMax_2D_directional(image)
...
Imax = find_maxima(image);
%disp('Imax');
Imin = find_minima(image);
%disp('Imin');


%remove saddle points
ind = find (Imax & Imin == 1);
Imax(ind) = 0;
Imin(ind) = 0;
...
sizeindmax = numel(indmax);
sizeindmin=numel(indmin);

param_alpha_max_tab=zeros(1,sizeindmax);
param_beta_max_tab=zeros(1,sizeindmax);
param_gamma_max_tab=zeros(1,sizeindmax);
param_alpha_min_tab=zeros(1,sizeindmin);
param_beta_min_tab=zeros(1,sizeindmin);
param_gamma_min_tab=zeros(1,sizeindmin);
...
%    end
   ...
   param_alpha_max_tab(i)= param_alpha;
   param_beta_max_tab(i)= param_beta;
   param_gamma_max_tab(i)= param_gamma;
...
for i=1:sizeindmin
   ...
   param_alpha_min_tab(i)= param_alpha;
   param_beta_min_tab(i)= param_beta;
   param_gamma_min_tab(i)= param_gamma;
%    if x1 == xi
%         param_beta = param_gamma * ((y3-yi)*(x1-xi) - (x3-xi)*(y1-yi))/((x2-xi)*(y1-yi)-(x1-xi)*(y2-yi));
%         param_alpha = (-param_beta*(y2-yi) + param_gamma*(y3-yi))/(y1-yi);
%    else
