...
function [A,normA] = defineParameterMax_column(image)
...
for k=1:Ny
    column = transpose(image(:,k));
    [indmin, indmax, indzer] = extr(column);
...
%                 beta(j+1)  = beta(j);           
%             end
            A(Nx*(k-1)+i,Nx*(k-1)+ind(j)) = 1;
            A(Nx*(k-1)+i,Nx*(k-1)+ind(j-1)) = d1(j)/d(j);
            A(Nx*(k-1)+i,Nx*(k-1)+ind(j+1)) = d2(j)/d(j);
            j=j+1;
        end
        if j == length(ind);
            break;
...
save('matriceDcolumn.mat','A','normA');
